				...
				{foreach from=$products item=product}
					<div class="bb-bid-item">
						...
						<div class="bb-list-txt">
							...
							{$lang.quantity}:&nbsp;{$product.amount}
							<div class="float-right">
								<a href="{"products.update&product_id=`$product.product_id`"|fn_url}" class="list-btn edit-btn">{$lang.edit}</a>
								<a class="list-btn delete-btn cm-confirm" href="{"products.delete?product_id=`$product.product_id`"|fn_url}">{$lang.delete}</a>
							</div>
						</div>
					</div>
				{/foreach}
