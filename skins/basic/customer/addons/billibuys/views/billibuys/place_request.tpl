...
<form name="bb_request_form" action="{"billibuys.view"|fn_url}" method="post" enctype="multipart/form-data">
	...
	<div class="form-field">
		...
		<textarea id="bb_request_desc" name="request[description]" size="255" maxlength="255" value="{$smarty.request.request.desc}" class="input-textarea-long">{$request.desc}</textarea>
	</div>

	<div class="form-field">
		<label for="bb_max_price" class="cm-trim cm-regexp">{$lang.max_price}</label>
		<input id="bb_max_price" type="text" name="request[max_price]" size="32" maxlength="32" value="{$smarty.request.request.max_price}" class="input-text" />
	</div>
